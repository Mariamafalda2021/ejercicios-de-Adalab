...
const App = () => {
  ...
  const handleMacarrones = ev => {
    setMacarrones(ev.currentTarget.checked);// En la que guardamos no el valor del input sino su propiedad checked, en macarrones con setMacarrones
  };
  ...
  return (
    <div>
      ...
      <form>
        ...
        <input type="button" value="marcar todos" onClick={handleAddAll} />
        <input type="button" value="desmarcar todos" onClick={handleResetAll} />
      </form>
      <p>{getMessage()}</p>
    </div>
  );
